...
{
    internal class RenameFileCommand : ISystemCommand
    {
        private readonly FileSystem _fileSystem;
        private readonly Session _session;


        public RenameFileCommand(FileSystem fs, Session session)
        {
            _fileSystem = fs;
            _session = session;
        }
        public bool CanExecute(string command) => Regex.IsMatch(command, "rename.file ([A-Za-zА-Яа-я1-9])+ ([A-Za-zА-Яа-я1-9])+");
        ...
        public async Task Execute(string command)
        {
            ...
            var file = list.Find(x => x.Name == oldName && x.Type == 0) ?? throw new NotFileException();
